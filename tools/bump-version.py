#!/usr/bin/env python3
"""Bump continuity's version: project.yml, a release commit, an annotated tag.
...
only writer, so the three things that have to move together always do:
  ...
  vX.Y.Z                  the annotated tag, `continuity X.Y.Z`
...
def main() -> None:
    ...
    git("tag", "-a", f"v{text}", "-m", f"continuity {text}")
